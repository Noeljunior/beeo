...
function gotrlink(field) {
    window.location.assign(field.getAttribute("href"));
}

function chooselanguage(elem) {
    var str = elem.options[elem.selectedIndex].value;
    str = URI(window.location.search).setSearch("language", str).normalizeSearch().toString();
...
function showbeeloading() {
    document.getElementById("loadingbee").style.display = 'block';
}
function hidebeeloading() {
    document.getElementById("loadingbee").style.display = 'none';
}
function showunknownerro() {
    var bee = document.getElementById("loadingbee");
    var errormsg = document.getElementById("errormsg");
    bee.setAttribute("href", window.location.search);
    errormsg.innerHTML = "Unknown error. Please reload the page!";
    bee.className = "loadingbee showerror bbacred";
    errormsg.className = "error";
}

function xhrnow(method, url, params, history) {
    var xhr = new XMLHttpRequest();
    xhr.onreadystatechange = function(response) {
        if (xhr.readyState == 4 && xhr.status == 200) {
            /* update history if we request was a GET */
            ...
            /* update the inner body */
            if (xhr.responseXML.body.getElementsByClassName("bkmright").length === 1
             && document.body.getElementsByClassName("bkmright").length === 1) {
                document.body.getElementsByClassName("bkmright")[0].innerHTML =
                    xhr.responseXML.body.getElementsByClassName("bkmright")[0].innerHTML;
                /* update selected menu item */
                var menuitems = document.getElementById("menuitems").children;
                var seledm = getParameterByName("m", url);
                for (var i = 0; i < menuitems.length; i++) {
                    var menuitem = menuitems[i].children[0];
                    menuitem.className = menuitem.className.replace("selected", "");
                    if (getParameterByName("m", menuitem.getAttribute("href")) == seledm) {
                        menuitem.className = menuitem.className + "selected";
                    }
                }
                /* scroll up */
                if (document.body.getElementsByClassName("bbreterror").length > 0) {
                    document.getElementById("bkminnerbody").scrollTop = 0;
                }
            } else {
                /* update the all body */
                document.body = xhr.responseXML.body;
                document.head = xhr.responseXML.head;
                //settheme();
            }
...
function getnow(elem, event) {
    if (event.button != 0)
        return true;
    return xhrnow("GET", elem.getAttribute("href"));
}

function postnow(elem, loadall) {
    loadall = typeof loadall !== 'undefined' ? true : false;
    var form = elem;
    do { form = form.parentNode;
    } while (form.tagName != undefined && form.tagName != "FORM");
    if (form.tagName != "FORM") return true;
    /* find the formaction: form itself, elem.formaction or the actual url */
    var url = window.location.search;
    if (form.getAttribute("action") != null)
        url = form.getAttribute("action");
    if (elem.getAttribute("formaction") != null)
        url = elem.getAttribute("formaction");
    /* get form parameters */
    var formData = new FormData(form);
    /* add the submitted button */
    formData.append(elem.name, elem.value);
    /* go request this */
    return xhrnow(form.getAttribute("method"), url, formData);
}


function qtdtsel(field, e) {
    var str = field.options[field.selectedIndex].value;
    /* if multiple selected, show, otherwise remove */
    var e = document.getElementById("qtdtselitems");

    if (str.toLowerCase().indexOf("multiple") >= 0) {
...
function taupdate(elem, ev, id) {
    document.getElementById(id).value = elem.innerHTML;
}
...
function removeqt(elem) {
    var torem     = elem.parentNode.parentNode;
    var container = elem.parentNode.parentNode.parentNode;
    container.removeChild(torem);
    /* the select element */
    var select    = document.getElementById("theoptionslist").getElementsByTagName("select")[0];
    /* the original list */
    var originallist = document.getElementById("theoriginallist");


    /* rebuild the options list: remove all the actual options */
    ...
    for (var i = 0; i < container.children.length; i++) {
        ...
        used[item.getAttribute("oid")] = true;
    }

    /* copy all over again except the used */
    for (var i = 0; i < originallist.children.length; i++) {
        if (used[originallist.children[i].getAttribute("oid")]) continue;
        select.appendChild(originallist.children[i].cloneNode(true));
    }
}

function addqt(elem) {
    /* the add element */
    var addelem   = elem.parentNode.parentNode;
    /* the container of everything */
    var container = addelem.parentNode;
    /* the copycat */
    var example   = document.getElementById("thehidden").cloneNode(true);
    /* the select element with the selected item to add */
    var select    = document.getElementById("theoptionslist").getElementsByTagName("select")[0];
    /* and the selected item */
    var seleditem = select.options[select.selectedIndex];


    /* set texts */
    example.getElementsByClassName("tl")[0].innerHTML = seleditem.getAttribute("txtl");
    example.getElementsByClassName("tr")[0].innerHTML = seleditem.getAttribute("txtr");
    /* set id the to hidden input */
    example.getElementsByTagName("input")[0].value = seleditem.value;
    /* set the original id on select list */
    example.setAttribute("oid", seleditem.getAttribute("oid"));
    ...
    container.insertBefore(example, addelem);

    /* remove the option from the options list */
    select.removeChild(seleditem);

}

function rorderqt(elem, dir) {
    /* the me.myseld element */
    var me        = elem.parentNode.parentNode.parentNode.parentNode;
    /* the container of everything */
    var container = me.parentNode;

    if (dir == "up") { /* going up */
        var swapwith = me.previousElementSibling;
        if (swapwith == null) /* the first */
            return;
        container.insertBefore(me, swapwith);
    } else
    if (dir == "down") {
        var swapwith = me.nextElementSibling;
        if (swapwith.getElementsByTagName("input").length == 0) /* the last */
            return;
        container.insertBefore(swapwith, me);
    }
}

function checkduplicates(container) {
    // check display:none!
    var selectcontainer = container.getElementsByClassName("checkduplicates")[0];
    var select = selectcontainer.getElementsByTagName("select")[0];
    ...
    /* for each added item, disable the option */
    for (var i = 0; i < container.children.length; i++) {
        var item = container.children[i];
        if (item.className.indexOf("checkduplicates") >= 0)
            continue;
        if (item.style.display === "none")
            continue;
        var valueid = item.getElementsByTagName("input")[0].value

        /* BIG O = O^2 ! ! ! javascript? */
...
function addev(elem) {
    var select    = elem.parentNode.parentNode.getElementsByTagName("select")[0];
    var seleditem = select.options[select.selectedIndex];
    var url = window.location.search + "&" + seleditem.getAttribute("golink") + "=" + seleditem.value;
...
function getCookieArray(cname) {
    try {
        ...
        var servname = logininfo.getAttribute("servname");
        var name = servname + cname + "=";
        var ca = document.cookie.split(';');
        for(var i=0; i<ca.length; i++) {
            var c = ca[i];
            while (c.charAt(0)==' ') c = c.substring(1);
            if (c.indexOf(name) == 0) {
                var res = JSON.parse(decodeURIComponent(c.substring(name.length,c.length)));
                if (res.constructor != Object)
                    return {}
                return res;
            }
        }
    } catch (e) { console.log("upg");
        return {};
    }
    return {};
}
function setCookieArray(cname, val) {
    try {
        var logininfo = document.getElementById("logininfo");
        var servname = logininfo.getAttribute("servname");
        cname = servname + cname;
        document.cookie = cname + "=" + encodeURIComponent(JSON.stringify(val));
...
function settheme() {
    var themeschooser = document.getElementById("themeschooser");
    if (!themeschooser)
        return;
    var styles    = document.getElementsByName("csstheme");
    var logininfo = document.getElementById("logininfo");
    for (i = 0; i < styles.length; i++) {
        styles[i].disabled  = true;
        if (themeschooser.options[themeschooser.selectedIndex].text == styles[i].getAttribute("title")) {
            styles[i].disabled  = false;
        }
    }

    var user = logininfo.getAttribute("userid");
    var role = logininfo.getAttribute("roleid");
    var theme = themeschooser.options[themeschooser.selectedIndex].text;

    /* update cookie */
    var themes = getCookieArray("UT");
    if (!themes[user])
        themes[user] = {};
    themes[user][role] = theme;
    setCookieArray("UT", themes);
}

function setrandomtheme() {
    var styles    = document.getElementsByName("csstheme");
    var logininfo = document.getElementById("logininfo");
    var rnd = Math.floor(Math.random() * styles.length) + 0;
    for (i = 0; i < styles.length; i++) {
        styles[i].disabled  = true;
        if (i == rnd) {
            styles[i].disabled  = false;
...
window.onload = initelems;*/


window.onpopstate = function(event) {
    xhrnow("GET", event.state.url, "", false);
};
